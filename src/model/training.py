...
logger = MLFlowLogger(
    ...
    log_model=True,
    run_name="overfit batch" if cfg.OVERFIT_BATCHES else "Training with no regularization",
)
...
model_checkpoint: ModelCheckpoint = ModelCheckpoint(
    dirpath=str(savedir / logger.experiment_id / logger.run_id / "artifacts"),
    filename="model_checkpoint-{epoch}-{val_loss:.4f}",
    monitor="valid_loss",
    verbose=True,
    save_last=None,
    save_top_k=1,
...
callbacks = [RichProgressBar(leave=True)] if cfg.OVERFIT_BATCHES else [early_stop_callback, RichProgressBar(leave=True)]
